    ...
    <div className="max-w-[1400px] mx-auto animate-in fade-in duration-500">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          <StatsCard label="Total Quizzes" value={stats.totalQuizzes} icon={BookOpen} color="purple" />
          <StatsCard label="Intentos Totales" value={stats.totalIntentos} icon={TrendingUp} color="blue" />
          <StatsCard label="Estudiantes Únicos" value={stats.uniqueStudents} icon={Users} color="green" />
          <StatsCard label="Puntaje Promedio" value={`${stats.avgScore}%`} icon={Trophy} color="amber" />
        </div>
      )}
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center bg-white border border-gray-100 rounded-2xl p-1 gap-1 shadow-sm">
            <button
              ...
              className={cn("p-2 rounded-xl transition-all", view === "grid" ? "bg-purple-600 text-white shadow-md shadow-purple-200" : "text-gray-400 hover:text-gray-600")}
            >
              <LayoutGrid size={20} />
            </button>
            <button
              onClick={() => setView("list")}
              className={cn("p-2 rounded-xl transition-all", view === "list" ? "bg-purple-600 text-white shadow-md shadow-purple-200" : "text-gray-400 hover:text-gray-600")}
            >
              <List size={20} />
            </button>
          </div>
          ...
          <button
            onClick={load}
            className="p-2.5 bg-white border border-gray-100 rounded-2xl text-gray-400 hover:text-purple-600 hover:border-purple-200 transition-all shadow-sm"
            title="Recargar"
          >
            <RotateCcw size={18} />
          </button>
